...
var theSampleLPString = "Maximize z = 87x1 + 147x2 + 258x3 subject to" + cr + "x1 + 2x2 + 3x3 <= 90" + cr + "15x1 + 21x2 + 30x3 <= 1260" + cr + "x1 + x2 + x3 <= 84";



var theInstructionString = "Remarques:" + cr + "(1) Les noms de variable doivent commencer par des lettres." + cr + "(par exemple, p, x1, perte, z, s, t, u ...)" + cr + "(2) Pour les entrées de fraction, gardez la variable à droite." + cr + "(ex. (1/3) x et non x / 3)" + cr + "(3) Chaque variable que vous utilisez doit apparaître dans la fonction objectif, (mais pas" + cr + "nécessairement dans les contraintes)." + cr + "(ex. p = 0x + 2y + 0z) " + cr + "(4) Les mots 'maximiser' (ou 'minimiser') et 'sous réserve de' doivent apparaît." + cr + "(5) Chaque inégalité doit être sur sa propre ligne, comme indiqué. " + cr + "(6)Inutile de saisir les contraintes par défaut: x> = 0, y> = 0, z> = 0 etc. "
...
function simplexMethod(InMatrix, rows, cols) {
...

	{
		...
		while (checkingForZeros) {
			...
			if (roundSigDig(InMatrix[theRow][cols], maxSigDig) == 0) InMatrix[theRowx][cols] = 0;
...
function SetupTableau() {
	...
	if (check == -1) {
		...
		document.theSpreadsheet.output.value = "Cela ne me semble pas être un problème de programmation linéaire!" + cr + cr + "Appuyez sur Exemple pour voir comment en saisir un.";
		okToRoll = false;
		return (666);
	}
...
function displayFinalStatus() {
	...
	if (TableauNumber > maxSteps) document.theSpreadsheet.expr.value = "Aucune solution optimale trouvée après 50 étapes. Avorté."

	else if (singular) document.theSpreadsheet.expr.value = "Aucune solution optimale n'existe pour ce problème."

	else

	{

		document.theSpreadsheet.expr.value = "Solution Optimale : " + objectiveName + " = ";
		...
		if ((fractionMode) || (integerMode)) document.theSpreadsheet.expr.value += toFrac(roundSigDig(objectiveVal, 15), maxDenom, tol) + "; ";
		else
...
function doIt() {
...

	{
		...
		document.theSpreadsheet.expr.value = "Appuyez sur 'Résoudre' pour résoudre le problème donné.";
